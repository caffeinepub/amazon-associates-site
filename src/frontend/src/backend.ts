...
export class Backend implements backendInterface {
    ...
    async getCategoryById(arg0: CategoryId): Promise<Category> {
        if (this.processError) {
            try {
    ...
    async getRecommendedProducts(): Promise<Array<Product>> {
        if (this.processError) {
            try {
                const result = await this.actor.getRecommendedProducts();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
